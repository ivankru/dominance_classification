...
class MedNeXt(nn.Module):
    ...
    def __init__(self, 
        ...
        self.conv_out = nn.Conv3d(in_channels = 512, out_channels = 1, kernel_size = 1)
        self.lin_out = nn.Linear(in_features=2 * 14 * 14, out_features=n_classes, bias=True)
...
if __name__ == "__main__":
    ...
    with torch.no_grad():
        # print(network)
        x = torch.zeros((4, 3, 32, 224, 224)).cuda()
        y0 = network(x)
        print(y0.shape)

        # conv1 = nn.Conv3d(in_channels = 512, out_channels = 1, kernel_size = 1).cuda()
        # lin1 = nn.Linear(in_features=2 * 14 * 14, out_features=1).cuda()
