...
from utils import *
from dataset_class import HeartLR_simple_main, HeartLightningModule, HeartLR_simple_main_frames
...
def get_pred_lists_for_table(models_weighted_pred_dict: dict, models_weighted_true_dict: dict) -> Tuple[List[str],
    ...
    return labels_list, confidence, std_list, true_list, true_confidence
...
def get_weighted_study_pred(series_weighted_preds_dict: dict, dataset_study_series_dict: dict) -> dict:
    ...
    for study in dataset_study_series_dict.keys():
        ...
        study_weighted_preds_dict[study] = [weighted_pred_label, weighted_pred, weighted_pred_std]
...
def get_study_preds_for_model(models_folder: str, model: Any, dataloader: DataLoader, best_model_loss: str,
                              ...
                              model_type: Optional[List[str]] = None, dataset_series_dict: Optional[List[dict]] = None,
...
def main(
    ...
    # loading of information about splitting into folds of the main dataset
    ...
    for i in range(5):
        ...
        test_dataset_rca_frames = Subset(dataset_rca_frames, get_studyid_nums(test_studies, dataset_rca_frames.studyid_ordernum_dict))
        test_dataset_lca_frames = Subset(dataset_lca_frames, get_studyid_nums(test_studies, dataset_lca_frames.studyid_ordernum_dict))

        test_dataset_rca_video = Subset(dataset_rca_video, get_studyid_nums(test_studies, dataset_rca_video.studyid_ordernum_dict))
        test_dataset_lca_video = Subset(dataset_lca_video, get_studyid_nums(test_studies, dataset_lca_video.studyid_ordernum_dict))

        dataloader_rca_video = DataLoader(
            test_dataset_rca_video,
            batch_size=1, 
        ...
        print(len(study_weighted_pred_dict_occlusion.keys()),len(true_list_occlusion),len(labels_list_occlusion),len(confidence_occlusion),len(std_list_occlusion),len(true_list_dominance),len(labels_list_dominance),len(confidence_dominance),len(std_list_dominance))
