...
def print_num_of_slices(projection_list: List[dict], phase_name: Optional[str] = None) -> None:
    ...
    for elem in projection_list: 
        if elem['dominance_type'] == 'left': 
            left_frames_c += 1

        elif elem['dominance_type'] == 'righ': 
            right_frames_c += 1

    print(f'{phase_name}: \n left frames = {left_frames_c}')
    print(f'{phase_name}: \n right frames = {right_frames_c}')
...
def main(
    ...
    # creating a dataset
    dataset = HeartLR_simple_main(dataset_folder, features=features,
    ...
    print("How many Left_Dominance:", count_label_0)
    print("How many Right_Dominance:", count_label_1)
    ...
    for i in range(len(folds['train'])):
        ...
        # increasing the size of training data by combining with validation data  
        ...
        for best_model, criterion in zip([best_model_loss], ['loss']):
            ...
            model.load_state_dict(torch.load(os.path.join(models_folder, best_model), map_location=device))
